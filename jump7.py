...
class Stick_Man:
    def __init__(self):       
        self.crouch = image.load('crouch.png').convert()
        self.fall = image.load('stand.gif').convert()
        self.jumping_right = image.load('jumping.jpg').convert()
        self.jumping_left = transform.flip(self.jumping_right, True, False)
        self.stand = image.load('fall.jpg').convert()
